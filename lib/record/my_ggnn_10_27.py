##################################################################
# 10.27改动
# 加了selfmodal的消息聚合，除此之外把，sendmessage的全连接移到了for循环的最上面
# 加了refine object时的跨模态抽取
##################################################################
...
class GGNN(nn.Module):
    ...
    def forward(self, rel_inds, ent_cls_logits, obj_fmaps, vr):
        ...
        for t in range(self.time_step_num):
            ...
            if refine_obj_cls:
                edges_img2ont_ent = self.crossmodal_img2ont_ent(message_send_img_ent, message_send_ont_ent)
                edges_ont2img_ent = self.crossmodal_ont2img_ent(message_send_ont_ent, message_send_img_ent)
            else:
            ...
            #模态内连接
            attention_ont_ent = self.selfmodal_ont_ent(message_send_ont_ent, message_send_ont_ent)
            attention_ont_pred = self.selfmodal_ont_pred(message_send_ont_pred, message_send_ont_pred)
            attention_img_ent = self.selfmodal_img_ent(message_send_img_ent, message_send_img_ent)
            attention_img_pred = self.selfmodal_img_pred(message_send_img_pred, message_send_img_pred)

            #跨模态连接
            edges_img2ont_pred = self.crossmodal_img2ont_pred(message_send_img_pred, message_send_ont_pred)
            edges_ont2img_pred = self.crossmodal_ont2img_pred(message_send_ont_pred, message_send_img_pred)

            ####这是融合到图片和常识图中每个节点中信息，图片融合常识图，常识图融合图�?            
            message_fusing_img_ent = torch.mm(edges_img2ont_ent, message_send_ont_ent) #N*151 151*1024
            message_fusing_img_pred = torch.mm(edges_img2ont_pred, message_send_ont_pred)
            message_fusing_ont_ent = torch.mm(edges_ont2img_ent, message_send_img_ent) #151*N N*1024
            message_fusing_ont_pred = torch.mm(edges_ont2img_pred, message_send_img_pred)
            ...
            message_incoming_ont_ent = torch.stack(
                ...
                torch.mm(attention_ont_ent, message_send_ont_ent)]
            , 1)
            ...
            message_incoming_img_ent = torch.stack([
                ...
                torch.mm(attention_img_ent, message_send_img_ent)]
            , 1)
            ...
            message_incoming_img_pred = torch.stack([
                ...
                torch.mm(attention_img_pred, message_send_img_pred)]
            , 1)
